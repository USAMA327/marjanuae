  ...
  return (
    <div className="py-4 px-2 sm:px-8 w-full absolute z-30 text-black">
      <div className="flex justify-between items-center">
        {/* Logo */}
        <Link href={"/"}>
          <Image className="size-14 md:size-18 lg:size-20" src={Logo} alt="Al-Marjan" priority />
        </Link>
            ...
            <button  className="relative focus:outline-none">
              {user?.photoURL ? (
                <img
                  src={user.photoURL}
                  alt="Profile"
                  className="size-10  rounded-full border border-gray-300 bg-gray-300"
                />
              ) : (
      ...
      {isMenuOpen && (
        <div className="md:hidden mt-4 p-4 bg-white">
          <ul className="flex flex-col space-y-4 cursor-pointer text-black">
            {routes.map((item) => (
              <Link onClick={() =>setIsMenuOpen(false)} key={item.id} href={item.route}>
