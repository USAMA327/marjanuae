    ...
    <div className="grid gap-6 md:grid-cols-2">
      {locations.map((location: OfficeLocation, index: number) => (
        ...
        >
          ...
          <a href={`https://www.google.com/maps?q=${location.latitude},${location.longitude}`} target="_blank"  className="flex mt-2 cursor-pointer  items-center gap-1 ">
          <span className="icon-[ph--map-pin-line-thin] size-5"></span>
            <p className="text-gray-600">{location.address}</p>
          </a>
          

          <a href={`tel:${location.phone}`} className="flex mt-2 cursor-pointer  items-center gap-1 ">
